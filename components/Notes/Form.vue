<template>
		<div class="border-2 p-[1.5rem] rounded-md">
			<div
			 class="flex flex-col">
				<label :class="{'text-red-500': !isAValidNote && isSubmited && !noteInput.title.lengt }" class="my-[0.5rem]">
					Title*
				</label>
				<input 
					placeholder="Note Title" 
					class="ml-[0.25rem] border-2 border-[#00000033] p-[0.25rem] py-[0.5rem] rounded-[0.25rem] outline-none focus:border-blue-300"
					:class="{'border-red-500': !isAValidNote && isSubmited && !noteInput.title.length }"
					v-model="noteInput.title"
				/>
				<label :class="{'text-red-500': !isAValidNote && isSubmited && !noteInput.note.length }" class="mt-[1rem] mb-[0.5rem]">
					Details*
				</label>
				<textarea 
					placeholder="Note Details..." 
					class="ml-[0.25rem] border-2 border-[#00000033] p-[0.25rem] py-[0.5rem] rounded-[0.25rem] outline-none focus:border-blue-300"
					:class="{'border-red-500': !isAValidNote && isSubmited && !noteInput.note.length }"
				...
				<div class="flex justify-center mt-[1rem]">
					<button 
						...
						:class="{'bg-[#EBEBE4] hover:bg-[#EBEBE4] text-[#C6C6C6]': !isAValidNote}"
